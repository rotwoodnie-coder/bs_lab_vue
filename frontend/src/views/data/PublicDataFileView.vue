      ...
      <template #header>
        <div class="user-page-header">
          <div class="user-page-title-wrap">
            <span class="user-page-title">数据维护 > 公共素材库维护</span>
          </div>
        </div>
      </template>
      ...
      <div class="public-file-body">
        ...
        <div class="public-file-main">
          ...
          <el-table :data="tableData" :row-class-name="currentRowClassName" border stripe v-loading="loading" class="user-table">
            ...
            <el-table-column label="是否公开" width="120" align="center">
              <template #default="scope">
                <el-switch
...
import { fetchDataFileLogs, fetchDataFileTypes, fetchDataFilesForPublic, updateDataFile,updateDataFilePublic } from '../../api/index'
...
const queryForm = reactive({ keyword: '', isPublic: '', fileTypeId: '', status: 'y', pageNum: 1, pageSize: 10 })
...
const togglePublicStatus = async (row, value) => {
  ...
  try {
    ...
    const res = await updateDataFilePublic(row.fileId, payload)
    if (res.data.code === 200) {
...
const isVideoFile = (row) => {
  const raw = String(row?.fileUrl || row?.fileName || '').trim().toLowerCase()
  console.log(raw);
...
const loadItems = async () => {
  loading.value = true
  try {
    const res = await fetchDataFilesForPublic({
