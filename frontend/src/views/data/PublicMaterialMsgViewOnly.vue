      ...
      <template #header>
        <div class="user-page-header">
          <div class="user-page-title-wrap">
            <span class="user-page-title">数据维护 > 公共材料库查看</span>
          </div>
        </div>
      </template>
...
import { fetchDataDictItems, fetchMaterialMsgsPublic, updateMaterialMsg, fetchMaterialMsg, fetchMaterialLogs } from '../../api/index'
...
const queryForm = reactive({ keyword: '', isPublic: 'y', materialTypeId: '', status: 'y', pageNum: 1, pageSize: 10 })
...
const loadItems = async () => {
  loading.value = true
  try {
    const res = await fetchMaterialMsgsPublic({
...
const togglePublicStatus = async (row, value) => {
  ...
  try {
    ...
    const res = await updateMaterialMsg(row.materialId, payload)
    if (res.data.code === 200) {
