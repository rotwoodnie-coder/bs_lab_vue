...
const confirmSaveItem = async (index) => { try { await saveItem(index, { force: true }) } catch (error) { if (error !== 'cancel' && error !== 'close') ElMessage.error(error?.response?.data?.message || error?.message || '提交失败') } }
...
const handleNameBlur = async (index) => { const item = referenceList.value[index]; if (!item?.referenceId) return; await saveItem(index) }
const handleSourceBlur = async (index) => { const item = referenceList.value[index]; if (!item?.referenceId) return; await saveItem(index) }
const handleContentBlur = async (index) => { const item = referenceList.value[index]; if (!item?.referenceId) return; await saveItem(index) }
