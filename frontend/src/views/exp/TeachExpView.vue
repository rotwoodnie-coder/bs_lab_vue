    ...
    <el-card class="user-card">
      <template #header>
        <div class="user-page-header">
          ...
          <el-button type="primary" plain @click="goCoursePage">教材模式</el-button>
        </div>
      </template>
...
import { deleteExpStandard, fetchDataDictItems, fetchExpTeachesMy, fetchExpSimulators, fetchLatestExpTeachesDraft, updateExpTeachSimulator } from '../../api/index'
...
const query = reactive({ keyword: '', subjectId: '', status: '', expType: 'teach', pageNum: 1, pageSize: 10 })
...
const loadItems = async () => {
  loading.value = true
  try {
    const res = await fetchExpTeachesMy({ ...query, expType: 'teach' })
...
const goCoursePage = () => {
  router.push('/admin/exp/exp-teach/course')
}
...
<style scoped>
.standard-toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:nowrap;margin-bottom:12px;overflow-x:auto}.standard-toolbar-inline{display:flex;align-items:center;gap:8px;flex-wrap:nowrap;min-width:max-content}.standard-toolbar-right{display:flex;align-items:center;flex-shrink:0;margin-left:auto}.standard-search{width:240px;flex-shrink:0}.standard-status-select{width:160px;flex-shrink:0}.simulator-preview-frame{width:100%;height:72vh;border:0;border-radius:8px;background:#fff}@media (max-width:768px){.standard-search,.standard-status-select{width:auto}.standard-toolbar,.standard-toolbar-inline{width:100%}.simulator-preview-frame{height:60vh}}
</style>
