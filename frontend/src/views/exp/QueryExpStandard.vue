      ...
      <template #header>
        <div class="user-page-header">
          <div class="user-page-title-wrap">
            <span class="user-page-title">标准实验库</span>
          </div>
        </div>
      </template>
...
import { deleteExpStandard, fetchDataDictItems, fetchExpStandards, fetchLatestExpStandardDraft } from '../../api/index'
...
const query = reactive({ keyword: '', subjectId: '', status: 'y', expType: 'standard', pageNum: 1, pageSize: 10 })
...
const loadItems = async () => {
  loading.value = true
  try {
    const res = await fetchExpStandards({ ...query, expType: 'standard' })
