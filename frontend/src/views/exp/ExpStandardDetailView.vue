<template>
  <div class="page-card detail-page" :class="{ 'detail-page--embedded': !showBackButton }">
    <el-card class="user-card detail-card">
      <template #header>
        <div class="user-page-header create-header">
          ...
          <div v-if="showBackButton" class="create-header-actions">
            <!--
            ...
            <div v-if="videoList.length" class="video-grid read-grid">
              <div v-for="(item, index) in videoList" :key="item.seqId || item.fileId || index" class="video-card read-card">
                <div class="video-card__preview read-preview">
                  <video :src="resolveFileUrl(item.previewUrl || '')" controls playsinline preload="metadata"></video>
                </div>
            ...
            <div v-if="expMaterialList.length" class="material-grid read-grid read-material-grid">
              <div v-for="(item, index) in expMaterialList" :key="item.expMaterialId || item.materialId || index" class="material-card read-card read-material-card">
                <div class="material-card__preview read-preview read-material-preview">
                  <el-image v-if="resolveFileUrl(item.mainPicUrl)" :src="resolveFileUrl(item.mainPicPreviewUrl)" fit="contain" :preview-src-list="[resolveFileUrl(item.mainPicUrl)]" preview-teleported />
        ...
        <template v-else-if="activeStep === 5">
          <el-descriptions border :column="2">
            ...
            <el-descriptions-item label="危险提示" :span="2">
              <div class="rich-view rich-view--panel" v-html="normalizeListHtml(form.expDanger)" />
            </el-descriptions-item>
          </el-descriptions>
        </template>
...
const props = defineProps({
  expId: { type: [String, Number], default: '' },
  showBackButton: { type: Boolean, default: true }
})

const route = useRoute()
const router = useRouter()
const expId = ref(String(props.expId || route.query.expId || '').trim())
const sourcePage = ref(String(route.query.sourcePage || '').trim())
...
const normalizeListHtml = (value) => {
  ...
  const parts = text
    ...
    .map(item => item.trim())
    .filter(Boolean)
  if (!parts.length) return '<span class="empty-rich">-</span>'
  return parts.map(item => `<div class="list-line">${item}</div>`).join('')
}
...
const goBack = () => router.push(sourcePage)

watch(
  () => props.expId,
  (value) => {
    const nextExpId = String(value || route.query.expId || '').trim()
    if (nextExpId && nextExpId !== expId.value) {
      expId.value = nextExpId
      loadDetail()
    }
  }
)
...
<style scoped>
.detail-page--embedded{padding:0}
...
.step-list .rich-view :deep(img){margin-left:0;margin-right:auto;}
.rich-view :deep(ul), .rich-view :deep(ol){padding-left:22px;margin:8px 0}
.rich-view :deep(li){margin:4px 0}
.list-line{margin:0 0 8px}
.list-line:last-child{margin-bottom:0}
...
.video-grid .read-card{max-width:320px;width:100%;}
.read-preview{width:100%;height:220px;background:#fff;border-radius:10px;overflow:hidden;display:flex;align-items:center;justify-content:center}
.read-preview video{width:100%;height:100%;display:block;object-fit:contain;background:#fff}
...
.read-material-preview{background:#fff}
