...
const TAB_EXPERIMENTS = { key: 'experiments', label: '模拟', icon: 'flask-conical', route: '/experiments' }
const TAB_TASKS = { key: 'tasks', label: '任务', icon: 'clipboard-list', route: '/tasks', hasBadge: true }
...
// 学生/家长/教师/教研员统一 5 Tab：首页(信息流) · 模拟 · 任务 · AI助手 · 我的
// 角色差异只体现在「首页 feed 的范围/筛选」，而不是替换某个 Tab。
const BASE_TABS = [TAB_HOME, TAB_EXPERIMENTS, TAB_TASKS, TAB_CHAT, TAB_PROFILE]

const TAB_ADMIN = { key: 'admin', label: '管理', icon: 'layout-dashboard', route: '/admin', hasBadge: true }
...
export const useAppStore = defineStore('app', () => {
  ...
  const tabs = computed(() => {
    ...
    if (role === 'admin') {
      return BASE_TABS.map((tab) => (tab.key === 'tasks' ? TAB_ADMIN : tab))
    }
    if (role === 'parent') {
      // 家长暂无 AI 助手未读提醒
